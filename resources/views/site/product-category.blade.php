...
@section('css')
	<style>
		...
		#objetivos a:hover, .active a
		{
			background: #47521c;
			text-align: center !important;
			border-bottom: 2px solid #ba9e17 !important;
			color: #fff !important;
		}
			...
			<ul>
				@foreach($categories as $category)
					<li{{ ($slug == $category->slug) ? ' class=active' : '' }}>
						<a href="{{  route('produto_categoria', ['type' => $category->type->slug, 'category' => $category->slug]) }}">{{ $category->name }}</a>
					</li>
				@endforeach
			</ul>
		</div>
